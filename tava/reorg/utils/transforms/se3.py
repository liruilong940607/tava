...
def se3_exp_map(log_transform: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    ...
    A SE(3) matrix has the following form:
        ...
        [ 0 1 ] ,
        ```
    Note(ruilongli): A change from the original pytorch3D code.
    ...
    # return transform.permute(0, 2, 1)  # pytorch3D's original code
    return transform
...
def se3_log_map(
    ...
    # T = transform[:, 3, :3]  # pytorch3D's original code
    T = transform[:, :3, 3]
