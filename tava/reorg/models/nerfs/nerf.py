...
class NerfModel(nn.Module):
    ...
    def __init__(
        ...
        self.pos_enc = instantiate(pos_enc)
        self.view_enc = instantiate(view_enc)

        input_dim = self.pos_enc.latent_dim if pos_enc else 3
        condition_dim = self.view_enc.latent_dim if view_enc else 0
        self.nets = nn.ModuleList([
            instantiate(net, input_dim=input_dim, condition_dim=condition_dim)
            for net in nets
        ])
    ...
    def _query_mlp(self, viewdirs, samples, i_level, meta=None, randomized=True):
        ...
        raw_rgb, raw_density = self.nets[i_level](
            samples_enc, condition=viewdirs_enc
        )
