...
def _load_renderings(root_fp: str, subject_id: str, split: str):
    ...
    if not root_fp.startswith("/"):  
        # allow relative path. e.g., "./data/dnerf/"
    ...
    for i in range(len(meta["frames"])):
        ...
        timestamp = (
            frame["time"] if "time" in frame else 
            float(i) / (len(meta["frames"]) - 1)
        )
...
class SubjectLoader(CachedIterDataset):
    ...
    SUBJECT_IDS = [
        "bouncingballs", "hellwarrior", "hook", "jumpingjacks", 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.images, self.camtoworlds, self.focal, self.timestamps = (
