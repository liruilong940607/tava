...
class _generate_training_samples_nerf(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, rays_o, rays_d, aabb):
        ...
        rays_o = rays_o.contiguous().view(-1, 3)
        rays_d = rays_d.contiguous().view(-1, 3)
        nears, fars = _backend.generate_training_samples_nerf(
            rays_o, rays_d, aabb
        )
        nears = nears.reshape(input_shape)
        fars = fars.reshape(input_shape)
        return nears, fars


generate_training_samples_nerf = _generate_training_samples_nerf.apply
